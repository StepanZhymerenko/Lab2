...
public class Main {
    public static void main(String[] args) {
        ...
        serializeReader(reader1, "reader1.ser");
        serializeReader(reader2, "reader2.ser");

        // Deserialize
        Bookshelf deserializedBookshelf = deserializeLibrary("library.ser");
        Reader deserializedReader1 = deserializeReader("reader1.ser");
        Reader deserializedReader2 = deserializeReader("reader2.ser");

        // Display deserialized state
        System.out.println("\nDeserialized state:");
        System.out.println(deserializedBookshelf);
        System.out.println(deserializedReader1);
        System.out.println(deserializedReader2);
    }
    ...
    private static void serializeReader(Object object, String filename) {
        try (ObjectOutputStream outputStream = new ObjectOutputStream(new FileOutputStream(filename))) {
            outputStream.writeObject(object);
            System.out.println("Serialized object saved to " + filename);
        } catch (IOException e) {
    ...
    private static Reader deserializeReader(String filename) {
        try (ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream(filename))) {
            Object object = inputStream.readObject();
